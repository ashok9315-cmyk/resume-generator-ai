export const RESUME_ANALYSIS_PROMPT = `You are an expert resume analyzer. Extract and structure the following resume text into clear sections.
...
export const RESUME_OPTIMIZATION_PROMPT = `You are an ATS (Applicant Tracking System) optimization expert. Optimize the following resume sections for maximum ATS compatibility while maintaining authenticity.
...
export const RESUME_FORMAT_PROMPT = `Convert the following optimized resume content into clean, ATS-compatible HTML format.
...
export const COVER_LETTER_MATCHING_PROMPT = `Analyze the following job description and resume to identify the most relevant experience and skills to highlight.
...
export const COVER_LETTER_PERSONALIZATION_PROMPT = `Create a personalized cover letter based on the job description and resume analysis.
...
export const COVER_LETTER_REFINEMENT_PROMPT = `Refine and polish the following cover letter to ensure it's professional, error-free, and impactful.
